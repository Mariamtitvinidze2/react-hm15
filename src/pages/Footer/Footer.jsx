...
return (
  ...
  <div className='w-full h-[50px] justify-center gap-[30px] border-b border-b-[#FFFFFF]  hidden max-md:flex'>
    <button className={`text-white ${FirstBTN ? "opacity" : "opacity-[20%]" } ${FirstBTN && "border-b-[#419EBB] && border-b-[4px]"} font-[spartan] text-[9px] font-[700] `} onClick={(e)=> FirstBTNHandle(e)}>OVERVIEW</button>
    <button className={`text-white ${SecondBTN ? "opacity" : "opacity-[20%]" } ${SecondBTN && "border-b-[#419EBB] && border-b-[4px]"} font-[spartan] text-[9px] font-[700]`} onClick={(e)=> SecondBTNHandle(e)}>Structure</button>
    <button className={`text-white ${ThirdBTN ? "opacity" : "opacity-[20%]" } ${ThirdBTN && "border-b-[#419EBB] && border-b-[4px]"}font-[spartan]  text-[9px] font-[700]`} onClick={(e)=> ThirdBTNHandle(e)}>Surface </button>
  </div>
  <div className='flex h-[80vh] max-xl:flex-col max-xl:h-[135vh] max-xl:items-center max-md:h-[68vh]'>
    ...
    <div className='w-[40%] mt-[50px] max-xl:flex max-xl:items-center max-xl:justify-center max-xl:gap-[100px] max-md:flex-col'>
      <div className='max-md:flex max-md:flex-col max-md:items-center max-md:w-[327px]'>
        ...
        <p className='w-[350px] font-[Spartan] font-normal text-[14px] mb-[20px] text-[white] max-xl:text-[11px] max-xl:w-[339px] max-md:w-[327px] max-md:text-center'>{FirstBTN ? data.overview.content : SecondBTN ? data.structure.content : ThirdBTN ? data.geology.content : "#"}</p>
        <div className='flex items-center gap-[8px] mb-[30px]'>
        <p className="font-[Spartan] font-normal text-[14px] tracking-[1px] text-[rgba(255,255,255,0.3)] max-xl:text-[12px]">Source: <a className="font-[700] underline text-[#ffffff]" target="_blank" href={FirstBTN ? data.overview.source : SecondBTN ? data.structure.source : ThirdBTN ? data.geology.source : "#"}>Wikipedia</a></p>
        <img className='' src={Picture} alt="" /> 
        </div>
      </div>
      <div className='flex  flex-col gap-[16px] max-md:hidden'>
      <button onClick={(e)=> FirstBTNHandle(e)} className={`flex gap-[20px] h-[48px] w-[350px] cursor-pointer ${!FirstBTN ? "bg-[#070724] BTN" : "bg-[#419EBB]"} items-center texts max-xl:w-[281px]`}>
      ...
      <button onClick={(e)=> SecondBTNHandle(e)} className={`flex gap-[20px] h-[48px] w-[350px] cursor-pointer  ${!SecondBTN ? "bg-[#070724] BTN" : "bg-[#419EBB]"} items-center  texts max-xl:w-[281px]`}>
      ...
      <button onClick={(e)=> ThirdBTNHandle(e)} className={`flex gap-[20px] h-[48px] w-[350px] cursor-pointer  ${!ThirdBTN ? "bg-[#070724] BTN" : "bg-[#419EBB]"} items-center  texts max-xl:w-[281px]`}>
